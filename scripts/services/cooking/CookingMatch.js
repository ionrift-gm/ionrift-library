/**
 * Contents- and charge-aware ingredient matcher for the cooking/feeding
 * abstraction. Ports the Monstrous Feast name/flag matching and the Respite
 * water/charge accounting (`_buildWaterOptions`, `_consumeItem`) into one
 * kernel primitive.
 *
 * A {@link IngredientSpec} never mints a discrete item: water and other
 * contents are matched structurally against what an actor already carries.
 */
...
/**
 * Read a flag for matching. With an explicit scope, read that namespace only;
 * otherwise fall back to the kernel-then-Respite dual lookup.
...
export const CookingMatch = {
    /**
     * Whether an item satisfies a spec. A name/accepts hit, a structured flag or
     * food-tag hit, or a contents-classification hit each qualify.
     * @param {Item|object} item
     * @param {IngredientSpec} spec
     * @param {{ actor?: Actor, system?: object }} [ctx]
     * @returns {boolean}
     */
    ...
    /**
     * Available units of a spec on an actor. For water/charge specs this counts
     * container charges and contained pints; otherwise it sums item quantity.
     * @param {Actor} actor
     * @param {IngredientSpec} spec
     * @param {{ system?: object }} [ctx]
     * @returns {number}
     */
    ...
    /**
     * First spec the actor holds in the required quantity.
     * @param {Actor} actor
     * @param {IngredientSpec[]} specs
     * @param {number} [quantity]
     * @returns {IngredientSpec|null}
     */
    ...
    /**
     * Consume up to `quantity` units of a spec from the actor. Decrements
     * quantity, charges, or contained children as appropriate. Returns the
     * number of units actually consumed (handles partial supply).
     * @param {Actor} actor
     * @param {IngredientSpec} spec
     * @param {number} quantity
     * @param {{ system?: object }} [ctx]
    ...
    /**
     * Consume up to `amount` units from a single item. Container, charge, and
     * plain-quantity branches mirror Respite's `_consumeItem`.
     * @private
     */
